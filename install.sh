...
readonly NOVA_INSTALL_PHASES="${NOVA_INSTALL_PHASES:-3}"
if [[ "$NOVA_INSTALL_PHASES" != "1" \
  && "$NOVA_INSTALL_PHASES" != "2" \
  && "$NOVA_INSTALL_PHASES" != "3" ]]; then
  nova_phase1_error "NOVA_INSTALL_PHASES must be 1, 2, or 3."
  exit 2
fi

nova_phase1_main "$@"
if [[ "$NOVA_INSTALL_PHASES" == "2" || "$NOVA_INSTALL_PHASES" == "3" ]]; then
  nova_phase2_main
fi
if [[ "$NOVA_INSTALL_PHASES" == "3" ]]; then
  nova_phase3_main
fi
